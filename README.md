...
## 目次
- [【立教大学 三宅/内山研 × 港区,アイザック社共同】高齢者esportsプロジェクト脳トレゲーム用リポジトリ](#立教大学-三宅内山研--港区アイザック社共同高齢者esportsプロジェクト脳トレゲーム用リポジトリ)
  - [目次](#目次)
  - [環境構築方法](#環境構築方法)
    - [1. 当リポジトリをクローン](#1-当リポジトリをクローン)
    - [2. Unityのインストール](#2-unityのインストール)
      - [2-1.プラットフォームの選択](#2-1プラットフォームの選択)
    - [3. Unityプロジェクトを開く](#3-unityプロジェクトを開く)
  - [ブランチ運用について](#ブランチ運用について)
  - [実際の作業手順](#実際の作業手順)
    - [1. `develop`を最新の状態に更新。](#1-developを最新の状態に更新)
    - [2. 作業用ブランチを作成し切り替える。](#2-作業用ブランチを作成し切り替える)
    - [3. 作業を開始する。](#3-作業を開始する)
    - [4. 作業用ブランチにコミットする。](#4-作業用ブランチにコミットする)
    - [5. 作業用ブランチをリモートリポジトリにプッシュする。](#5-作業用ブランチをリモートリポジトリにプッシュする)
    - [6. （作業完了後）`PullRequest`を作成する。](#6-作業完了後pullrequestを作成する)
  - [`PullRequest`を`develop`にマージする。](#pullrequestをdevelopにマージする)
    - [1. 当該のPullRequestの内容を確認する。](#1-当該のpullrequestの内容を確認する)
    - [2. 動作確認を行う。](#2-動作確認を行う)
    - [3. 動作確認が完了したら、`PullRequest`をマージする。](#3-動作確認が完了したらpullrequestをマージする)
  - [`PullRequest`の作成方法](#pullrequestの作成方法)
    - [1. GitHubのページにアクセス](#1-githubのページにアクセス)
    - [2. `base`と`compare`を設定](#2-baseとcompareを設定)
    - [3. `PullRequest`を投稿する。](#3-pullrequestを投稿する)
    - [4. 投稿完了](#4-投稿完了)
